	...
	void RenderObject(rw_clustering_ptr<Shader_Object>& shader, RenderObjectParams&& params)
	{
		if (params.useWorldSpaceTransform && m_ModelMatrix.isValid())
		{
			...
			if (params.prevViewProjection != nullptr && m_PrevModelMatrix.isValid())
			{
				...
				m_PrevModelMatrix.oneTimeWrite(&Transform::SetModelMatrtix, m_ModelMatrix.get()->GetModelMatrix());
			}
		}
						...
						if (m_TextureMap.contains((TexType)texType))
						{
							...
							if (auto texMap = m_TextureMap.at((TexType)texType)[materialIndex].get()) 
							{
								...
								case Albedo:
									...
									shader.oneTimeWrite(&Shader_Object::SetTextureUnitStructArr, "material", 0, "albedoMap");
									break;
								case Metallic:
									//shader->SetTextureUnitStructArr("material", 0, "metallicMap");
									shader.oneTimeWrite(&Shader_Object::SetTextureUnitStructArr, "material", 0, "metallicMap");
									break;
								case Roughness:
									//shader->SetTextureUnitStructArr("material", 0, "roughnessMap");
									shader.oneTimeWrite(&Shader_Object::SetTextureUnitStructArr, "material", 0, "roughnessMap");
									break;
								case Normal:
									//shader->SetTextureUnitStructArr("material", 0, "normalMap");
									shader.oneTimeWrite(&Shader_Object::SetTextureUnitStructArr, "material", 0, "normalMap");
									break;
								case Parallax:
									//shader->SetTextureUnitStructArr("material", 0, "parallaxMap");
									shader.oneTimeWrite(&Shader_Object::SetTextureUnitStructArr, "material", 0, "parallaxMap");
									break;
								case Glow:
									//shader->SetTextureUnitStructArr("material", 0, "glowMap");
									shader.oneTimeWrite(&Shader_Object::SetTextureUnitStructArr, "material", 0, "glowMap");
									break;
								case Displacement:
									//shader->SetTextureUnit("displacementMap");
									shader.oneTimeWrite(&Shader_Object::SetTextureUnit, "displacementMap");
									//ToDo: Add dispFactor through material or something
									shader->SetVariable("dispFactor", 0.2f);
									break;
								case Noise:
									//shader->SetTextureUnit("noise");
									shader.oneTimeWrite(&Shader_Object::SetTextureUnit, "noise");
		...
		shader.oneTimeWrite(&Shader_Object::ResetTextureUnit, resetToTexUnit);
	}
