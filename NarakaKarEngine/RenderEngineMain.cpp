...
struct RenderEngineMain::Impl
{
	...
	{
		auto rUtil = [&](const int iTexIndex, const int oTexIndex, Texture* intTex, Texture* outTex, const unsigned size)
		{
			maxReduceCompShader->UseShader();
			outTex->UseTextureReadWrite(0, false);
			intTex->UseTextureArray(0);
			GLuint location = glGetUniformLocation(maxReduceCompShader->GetShaderID(), "inTexIndex");
			glUniform1i(location, iTexIndex);
			location = glGetUniformLocation(maxReduceCompShader->GetShaderID(), "outTexIndex");
			glUniform1i(location, oTexIndex);

			unsigned dSize = std::max(size / 32, 1u);
			maxReduceCompShader->Dispatch(dSize, dSize);
		};

		unsigned tSize = ScreenWidth / 2;
		rUtil(inTexId, 0, texture, reduceTextures, tSize);
	...
	{
		RKAdvect(velocityTex, fieldst, readVelId, texId[0], texId[1], 0.5f);
		RKAdvect(velocityTex, fieldst, readVelId, texId[1], texId[2], -0.5f);
