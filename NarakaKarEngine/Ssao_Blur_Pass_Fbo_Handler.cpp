...
struct Ssao_Blur_Pass_Fbo_Handler::Impl
{
...
Ssao_Blur_Pass_Fbo_Handler::Ssao_Blur_Pass_Fbo_Handler(const GLuint& width, const GLuint& height) : m_pImpl{ new Impl(width, height) } {}

void Ssao_Blur_Pass_Fbo_Handler::WriteToFBO() const
{
	Pimpl()->WriteToFBO();
}

void Ssao_Blur_Pass_Fbo_Handler::AttachFBOToTextureUnit(const GLenum& textureUnit) const
{
	Pimpl()->AttachFBOToTextureUnit(textureUnit);
}

void Ssao_Blur_Pass_Fbo_Handler::ResizeFBO(const GLuint& width, const GLuint& height)
{
	Pimpl()->ResizeFBO(width, height);
}

const GLuint& Ssao_Blur_Pass_Fbo_Handler::GetFBOWidth() const
{
	return Pimpl()->m_Fbo->GetWidth();
}

const GLuint& Ssao_Blur_Pass_Fbo_Handler::GetFBOHeight() const
{
	return Pimpl()->m_Fbo->GetHeight();
}

const GLuint& Ssao_Blur_Pass_Fbo_Handler::GetFBOBuffer(const GLuint& bufferIndex) const
{
	return Pimpl()->m_Fbo->GetBuffer(bufferIndex);
}

Ssao_Blur_Pass_Fbo_Handler::~Ssao_Blur_Pass_Fbo_Handler() = default;