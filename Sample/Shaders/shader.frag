...
{
    vec3 bumpedNormal 		= texture(material.normalMap, texCoord).xyz;
...
vec2 CalcParallaxMapping(vec3 viewDir, vec2 texCoord)
{
	...
	float currDepthMapValue = texture(material.parallaxMap, currTexCoords).r;
	...
	while(currLayerDepth < currDepthMapValue)
	{
		...
		currDepthMapValue 	= texture(material.parallaxMap, currTexCoords).r;  
	...
	float beforeDepth 		= texture(material.parallaxMap, prevTexCoords).r - currLayerDepth + layerDepth;
...
void main()												
{
	...
	vec3 newNormal 			= CalcBumpedNormal(newTexCoord);
	
	vec3 albedo 			= texture(material.albedoMap, newTexCoord).rgb;
	float metallic 			= texture(material.metallicMap, newTexCoord).r;
	float roughness 		= texture(material.roughnessMap, newTexCoord).r;
